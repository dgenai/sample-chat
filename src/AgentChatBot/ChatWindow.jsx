...
import { useWallet } from "@solana/wallet-adapter-react"; // Hook to access wallet context

export default function ChatWindow({ agent, onBack, apiBase }) {
  const theme = useTheme();
  const { publicKey } = useWallet(); // Get connected wallet public key
  ...
  const sendMessage = async () => {
    ...
    // Include the connected user's wallet public key in metadata when available
    const metadata = {
      source: "web-client",
      timestamp: new Date().toISOString(),
    };

    // Only attach the wallet public key if one is connected
    if (publicKey) {
      metadata.userWalletPubKey = publicKey.toBase58();
    }

    const payload = {
      agent: `${"https://api.dgenai.io".replace(/\/$/, "")}/a2a/${agent.id}/.well-known/agent-card.json`,
      ...
      >
        ...
        >
          <Typography sx={{ fontWeight: 600 }}>
            [ Connected to <span style={{ color: accent }}>{agent.name}</span> ]
          </Typography>
